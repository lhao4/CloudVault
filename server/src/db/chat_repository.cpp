...
std::vector<ChatMessageRecord> ChatRepository::fetchUndeliveredMessages(int receiver_id) {
    auto conn = db_.acquire();
    const char* sql =
        "SELECT om.sender_id, om.receiver_id, su.username, ru.username, om.content, "
        ...
        "WHERE om.receiver_id = ? AND om.delivered = 0 AND om.msg_type = ? "
    ...
    MYSQL_STMT* stmt = *stmt_opt;
    uint32_t msg_type = static_cast<uint32_t>(MessageType::CHAT);
    MYSQL_BIND params[2]{};
    params[0].buffer_type = MYSQL_TYPE_LONG;
    params[0].buffer = &receiver_id;
    params[1].buffer_type = MYSQL_TYPE_LONG;
    params[1].buffer = &msg_type;
...
bool ChatRepository::markMessagesDelivered(int receiver_id) {
    ...
    const char* sql =
        ...
        "WHERE receiver_id = ? AND delivered = 0 AND msg_type = ?";
    auto stmt_opt = prepareStmt(conn.get(), sql);
    if (!stmt_opt) {
        return false;
    }
